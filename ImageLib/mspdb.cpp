...
void tryLoadLibrary(const char* mspdb)
{
	...
	if(GetModuleFileNameA(modMsPdb, modpath, 260) < 260)
	{
		...
		while(p > modpath && p[-1] != '\\')
			p--;
		strcpy_s ( p, sizeof(p), "mspdbsrv.exe");
...
bool tryLoadMsPdb ( const char* version, const char* mspdb, const char* path = 0)
{
	...
	size_t installDirLenght = 0;
	
	if ( ! getInstallDir ( version, installDir, sizeof(installDir) ) )
		return false;

	installDirLenght += strlen ( installDir );

	char* p = installDir + installDirLenght;

	if (p[-1] != '\\' && p[-1] != '/')
		*p++ = '\\';

	if ( path ){
		strcpy_s ( p, sizeof ( installDir ) - installDirLenght, path );
		installDirLenght += strlen(p);
		p += installDirLenght;
	}

	strcpy_s ( p, sizeof ( installDir ) - installDirLenght , mspdb);
