...
// Start Andrew.W Worobow 2015 

	pub->detail->x86.displacementOffset = inter->displacementOffset;
	pub->detail->x86.immediateOffset = inter->immediateOffset;
	pub->detail->x86.immediateSize = inter->immediateSize;
	pub->detail->x86.displacementSize = inter->displacementSize;

	pub->detail->x86.HasDisplacement= inter->HasDisplacement;
	pub->detail->x86.HasImmediate = inter->HasImmediate;

// End Andrew.W Worobow 2015 
		...
		MCInst *instr, uint16_t *size, uint64_t address, void *_info)
{
	...
	if (instr->flat_insn->detail) {
		...
		// Andrew.W Worobow 2015
		insn.HasDisplacement = false;
		insn.HasImmediate = false;
		// end Andrew.W Worobow
	}
